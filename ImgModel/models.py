...
class oracle_teamd(Model_oracleDB_teamd):
    #food 테이블 연결
    def up_photo_DB(self, normal_id, foodnum, category, candidate_predictions, top_4_probabilities):
        conn = self.myconn()
        cursor = conn.cursor()
        ...
        sql = """
            ...
            WHERE 
              nm.normal_id = :normal_id


            """
        cursor.execute(sql, upphotoid=upphotoid, normal_id=normal_id, foodnum=foodnum, category=category,
